...
- **[setup-sandcastle](./setup-sandcastle/SKILL.md)**: Scaffold a `.sandcastle/` directory that mounts the skills into a Docker container and hands `ready-for-agent` issues to them, so `implement` runs AFK on an isolated branch.
